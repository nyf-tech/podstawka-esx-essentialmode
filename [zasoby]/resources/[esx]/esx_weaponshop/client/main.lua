...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('esx_weaponshop:getAmmoShop', function(shopAmmoItems)
		for k,v in pairs(shopAmmoItems) do
			Config.Zones[k].Ammo = v
		end
	end)
...
RegisterNetEvent('esx_weaponshop:sendAmmoShop')
AddEventHandler('esx_weaponshop:sendAmmoShop', function(shopAmmoItems)
	for k,v in pairs(shopAmmoItems) do
		Config.Zones[k].Ammo = v
	end
end)
...
function OpenAmmoMenu(zone)
	...
	for i=1, #Config.Zones[zone].Ammo, 1 do
		...
		table.insert(elements, {
			label = ('Munición de %s - <span style="color: green;">%s</span>'):format(item.label, _U('shop_menu_item', ESX.Math.GroupDigits(item.price))),
			price = item.price,
			weaponName = item.item
		})
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop', {
		title = _U('ammo_shop_menu_title'),
		align = 'top-left',
		elements = elements
	}, function(data, menu)
		ESX.TriggerServerCallback('esx_weaponshop:buyWeaponAmmo', function(bought)
			if bought then
				DisplayBoughtScaleform(data.current.weaponName, data.current.price)
			else
				PlaySoundFrontend(-1, 'ERROR', 'HUD_AMMO_SHOP_SOUNDSET', false)
